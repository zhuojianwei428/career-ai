...
- 制限：ログイン不要・**1日2回/人**（ブラウザ Cookie で同日カウント、サーバ側強制）。超過は `429` を返しフロントでボタンを無効化。全ツールページで共有。※ Cookie 消去で回避可能（無料・ログイン不要とのトレードオフ）。本格運用は IP 制限 / KV ストア。単体テストは `test-ratelimit.mjs`
