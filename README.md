...
- **捏造禁止ルール**：`api/generate.js` の `NO_FABRICATION_RULES` に一本化し、**構造化契約（`buildPrompt`）と旧契約（`messages`）の両方に前置**する。片方だけだとツールによって捏造の有無が変わる（実際 `index.html` 以外の 4 ツールは旧契約を通っており、この規則が一切効いていなかった）。特に「企業のミッション・理念・スローガンを `「」` 付きで引用しない」「企業情報に無い数値を書かない」「文字数を自己申告しない」を明文化している（プロンプトに書いただけでは足りないので、実測で検証する）。
  ...
  - **穴は 1 文に 1 つまで**：実測（2026-09-21・実際の生成結果）で `では【事業内容への共感】【入社後に携わりたい業務】【志望の理由】を基に、実践を通して成長していきたいです。` のように**3 つの穴が 1 文に連結**された。ラベルは短くても、利用者が 3 つ同時に埋めるのは難しく、埋めた後も日本語として不自然になる。そこで「複数の穴を連結しない／穴は文の主語・目的語として置き、残りは別の文に分ける」を明示した。
  - ⚠️ **ただし「2 穴」の形は prompt では消えなかった**：`【事業内容への共感】から、【入社後に携わりたい業務】に取り組み、…` という締めの一文が、**3 ラウンドの規則追加（最後は観測した原句をそのまま悪例に書いた）を経ても逐字で再出現**した。理解の問題ではなく生成分布の問題なので、**層を変えて「サーバ側で確定判定」**する方式にした：`countMultiHoleSentences()` が 1 文に `【 】` が 2 つ以上ある文を数え、`multiHoleSentences` として**両方の契約のレスポンスに載せ**、`[gen][multi-hole]` にログを残す。UI は穴が残っているときだけ「同じ文に【 】が2つ以上ある箇所が n 件あります（1つずつクリックで埋められます）」と一言添える。**`notice` には入れない**（捏造ではなく読みやすさの問題なので利用者を驚かせない／`truncated` と混ぜない）。
  - この判断の根拠になった実測（本番・実モデルでの生成 19 件）：**鉤括弧 0/19・文字数の自己申告 0/19・前置き 0/19**（＝宣称に関わる問題は 0）に対し、**1 文 2 穴は旧契約で ~6/14**（自己PR に集中、新契約は 0/3）。**「宣称の合規」と「読みやすさの質」は分けて扱う**——前者は 0 でなければならないが、後者は程度問題。
  - **確認済みを前提に直した**：旧ルール 8 は「gBizINFO は要確認。同名の別法人の可能性があるため企業像を断定するな」だったが、**P0-0 以降は利用者が候補から選び法人番号まで確認している**。迷わせる記述は確認済みの事実と矛盾するので削除し、「登録情報は取得時点のスナップショットなので断定しすぎない」に置き換えた。UI 側の `setStatus` も `（要確認）` → `（確認済み・法人番号つき）` に修正。
...
- 回帰テスト：`node test-p1-baseline.mjs`（**187 項目**・ネットワーク不要）。gBizINFO のスタブが**実測した壊れ方（末尾スラッシュ＝500）を再現する**ので、同じ改修をするとテストが落ちる。ハンドラを実際に走らせて `companyContextUsed`・`notice`・`gbiz` の診断値まで確認する。`node test-p0.mjs`（**219 項目**）は確認フロー・不正な法人番号で fetch 0 回・実レート制限・文案・去歧義・字数・モバイルまで見る。

## 回帰テスト一覧（すべてネットワーク不要）

```
node test-p0.mjs              219 項目  P0-0 確認フロー / 文案 / 占位符 / 字数 / モバイル
node test-p1-baseline.mjs     187 項目  gBizINFO の URL・失敗の可観測性 / 捏造禁止規則 / 截断
