...
ok(/登録情報を確認して反映/.test(index), "P0-8: 企業情報の取得方法を具体的に（過大な言い方をしない）");
ok(/候補が表示される場合があります（該当する法人がない場合は表示されません）/.test(index) && !/の候補が表示されます。/.test(index), "P0-8: gBizINFO 候補は条件付きと明記（404 実測と整合）");
