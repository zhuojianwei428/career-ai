...
// 本文に残ると (a) 字数カウンタと矛盾して壊れて見える (b) 全文コピーでそのまま相手に渡る。
...
1. 企業固有の内容（事業内容・製品・サービス名・技術スタック・具体的な施策・課題・数値・固有名詞）は、「企業情報」または「事業内容」に明記されている場合のみ記述する。どちらにも記載が無いものは絶対に書かない。（「事業内容」は利用者本人が入力した会社の事業内容なので、事実として扱ってよい。ただし書かれていない細部を足さない。）
...
8. 企業名は必ず含める。ただし企業名以外の企業固有情報は「企業情報」「事業内容」由来のものだけにする。
...
11. 「きっかけ・共感した点」「入社後にやりたいこと」は、利用者本人が書いた言葉である。入力がある場合は、その内容を本人の言葉として本文に組み込み、対応する穴（【事業内容への共感】【志望の理由】【入社後に携わりたい業務】など）を残さない。入力が無い場合にだけ、その穴を【 】で残す。
12. 11 で入力を組み込むときも、書かれていない感情・動機・意図を付け足さない。本人の言葉を薄めて一般論（成長したい／貢献したい等）に置き換えることもしない。入力を超えて具体的にしない。`;

// 応募種別ごとの構成方針。FAQ「それぞれに最適な構成で生成します」の実装側の裏付け
// （2026-09-22 実測: scene は user prompt の1行に載るだけで、構成指示が一切無かった）。
// index の応募種別チップ（新卒/転職/バイト）に対応。手引き「書き分け」と同じ軸。
const SCENE_GUIDE = {
  "新卒": "学生時代の経験を軸に、それをこの仕事でどう活かすかへ接続する構成にする。",
  "転職": "前職の実績を軸に、入社後すぐに活きる即戦力としての接続を示す構成にする。",
  "バイト": "熱意と、勤務を続ける意思が伝わる構成にする。"
};

function buildPrompt(tool, f, merged) {
  const sceneGuide = SCENE_GUIDE[String((f && f["応募種別"]) || "").trim()] || null;
...
【出力ルール】
- 入力された応募種別・職種・文字数・トーンに従い、結論ファーストで具体的に作成する。${sceneGuide ? "\n- 構成の方針: " + sceneGuide : ""}
- 指定の文字数目安に収める。
- 本文のみを出力する。`;
  const ctxBlock = merged && merged.text ? merged.text : "なし";
  // 中小企業では gBizINFO が「法人名＋本店所在地」の 2 件しか返さず、完成度は利用者の入力が担う。
  // よって入力された素材は必ずプロンプトに載せる。載せ忘れると「利用者が書いたのに【 】が残る」
  // ＝ 価値提案（入力量＝完成度）が嘘になる。2026-09-22 実測でこの 3 フィールドが載っておらず、
  // 出力に事業内容が一切反映されない事故が起きたため、ここが唯一の追記ポイントだと明示しておく。
  const user = [
    "応募種別: " + (f["応募種別"] || ""),
    "職種: " + (f["職種"] || ""),
    "企業名: " + (f["企業名"] || ""),
    "経験・キーワード: " + (f["経験・キーワード"] || ""),
    "事業内容（利用者が入力した会社の事業内容。空欄なら未入力）: " + (f["事業内容"] || ""),
    "きっかけ・共感した点（利用者本人が入力した言葉。空欄なら未入力）: " + (f["共感した点"] || ""),
    "入社後にやりたいこと（利用者本人が入力した言葉。空欄なら未入力）: " + (f["入社後にやりたいこと"] || ""),
    "文字数: " + (f["文字数"] || ""),
    "トーン: " + (f["トーン"] || ""),
...
（※デモ文。本番はあなたの経歴から本番想定の回答を生成します。）`;
    ...
    // デモでも「入力があれば穴を残さない」を守る（本番と同じ約束を崩さない）
    const biz = (f["事業内容"] || "").trim();
    const empath = (f["共感した点"] || "").trim();
    const after = (f["入社後にやりたいこと"] || "").trim();
    const reason = empath || ((exp ? exp + "を通じて" : "") + "【志望の理由】");
    body =
`■第1段落（結論）：なぜ${company}なのか
${company}を志望する理由は、${reason}です。${biz ? "（事業内容：" + biz + "）" : ""}
...
（※デモ文。実際の AI はご入力から生成します。企業固有の内容は「企業情報」「事業内容」からのみ記述し、空欄は【 】で残します。文字数目安：${len}字／トーン：${tone}）`;
  }
