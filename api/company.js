...
export default async function handler(req, res) {
  ...
  res.status(200).json({
    ...
    diag: r.diag,
    // 文言は事実に合わせる。上流が 404 で返した「該当なし」は「データベースに接続できた上で
    // 『無い』と回答された」なので、接続故障と同じ文言にすると事実と食い違う（赤線: 表述と実装の不一致）。
    // 200 で空リストだった場合（個人事業主・屋号など）は従来どおりの説明を残す。
    message: r.items.length
      ? null
      : (r.diag && r.diag.notFound)
        ? "該当する企業が見つかりませんでした。入力された企業名と企業情報だけで生成します。"
